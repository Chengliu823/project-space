...
public class Algorithms implements MATSimAppCommand {
    ...
    private NetworkValidation.API api;
    ...
    @CommandLine.Option(names = "--max-calculation", description = "output folder for the route calculation", defaultValue = "2000")
    private double maxCalculations;
    ...
    public Integer call() throws Exception {
        ...
        List<initialList> basicList = new ArrayList<>();

        if (api == NetworkValidation.API.GOOGLE_MAP) {
            if (date == null) {
        ...
        } else if (api == NetworkValidation.API.HERE) {
            if (date == null) {
...
//              double departureTime = 3600;   // To test free speed travel time
               ...
               //     basicList.add(link.getId().toString(),link.getFreespeed()，);
               // }


                /*
                List<basicList> firstBasicList = new ArrayList<>();
                List<String> idList = new ArrayList<>();
                //List<validDataList> validDataLists = new ArrayList<>();

                for (Link link: route.links) {
                    idList.add(link.getId().toString());
                    firstBasicList.add(new basicList(link.getId().toString(),link.getFreespeed()));
                }

                Map<String, Integer> idMap = new HashMap<String, Integer>();
                for (String id : idList){
                    Integer idTimes = idMap.get(id);
                    idMap.put(id,(idTimes == null) ? 1:idTimes+1);
                }

                for (Map.Entry<String, Integer> val: idMap.entrySet()) {
 ...
 */
                ...
                if (calculation >= maxCalculations) {
                    break;
                }
            }
            tsvWriter.close();
        ...
        class initialList{
            ...
            public initialList(String id, double freeSpeed, double score) {
                this.id = id;
